...
function runFinished(data, code, done, timeout) {
  ...
  switch (language) {
    case 'Java': onJavaCodeRunned(data, code, done, timeout); break;
    case 'C++': onCPPCodeRunned(data, code, done, timeout); break;
  }
}

function onCPPCodeRunned(data, code, done, timeout) {
...
function onJavaCodeRunned(data, code, done, timeout) {
  var solution = data.solution,
      problem = data.problem,
      errorLog;
